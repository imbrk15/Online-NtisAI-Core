...
function HomeServiceCard({ link, icon, title, subtext, stats }) {
    ...
    const canAccess = hasPermission(link, 'view');

    if (!canAccess) {
        return (
            <div className="block group opacity-60 cursor-not-allowed">
                <div className="relative pt-[20px] pr-5 pb-[15px] pl-[15px] bg-white rounded-2xl shadow-md min-h-[130px] flex flex-col justify-end border-l-[8px] border-solid border-gray-400">
                    {/* Card Stats (Top-right) */}
                    <div className="absolute top-[1px] right-[5px] flex flex-col items-end z-10">
                        {stats.map((stat, index) => (
                            <div
                                key={index}
                                className="py-1 px-[5px] rounded-xl text-[12px] text-gray-500 whitespace-nowrap font-bold"
                            >
                                {stat.label}:{" "}
                                <strong className="text-gray-500">{stat.value}</strong>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-col">
                        <div className="text-4xl grayscale">{icon}</div>
